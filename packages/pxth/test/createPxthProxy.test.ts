...
describe('PxthProxy', () => {
	it('should modify segments array', () => {
		const proxy = createPxthProxy<{ hello: { world: string } }>(['hello', 'bye']);

		expect(proxy.hello.world[SegmentsToken]).toStrictEqual(['hello', 'bye', 'hello', 'world']);
	});

	it('should be possible to convert to json', () => {
		expect(JSON.stringify(createPxthProxy([]))).toBe('{"source":[]}');
		expect(JSON.stringify(createPxthProxy(['hello', 'world']))).toBe('{"source":["hello","world"]}');
	});
