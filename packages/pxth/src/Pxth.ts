	...
	boolean extends IsAny<T>
		? /* Handle arrays & tuples */
			...
			? {
					...
					[K in Extract<keyof NonNullable<T>, `${number}` | number>]: Pxth<
						NonNullable<T>[K] | Extract<T, null | undefined>
					>;
				} & {
					length: Pxth<NonNullable<T>['length'] | Extract<T, null | undefined>>;
				}
			: /* Handle objects */
				NonNullable<T> extends object
				? {
						[K in keyof NonNullable<T>]-?: Pxth<NonNullable<T>[K] | Extract<T, null | undefined>>;
					}
				: {}
		: any;
...
if (import.meta.vitest) {
	...
	describe('Pxth<T> type tests', () => {
		it('should correctly handle primitive types', () => {
			expectTypeOf<Pxth<string>>().toMatchTypeOf<Pxth<string | undefined>>();
			expectTypeOf<Pxth<string | undefined>>().not.toMatchTypeOf<Pxth<string>>();
		...
		it('should correctly work with objects', () => {
			expectTypeOf<Pxth<{ a: string; b: number }>>().toMatchTypeOf<Pxth<{ a: string }>>();
			expectTypeOf<Pxth<{ a: string; b: { c: number } }>>().toMatchTypeOf<Pxth<{ a: string }>>();
			expectTypeOf<Pxth<{ a: string }>>().toMatchTypeOf<Pxth<Record<string, string>>>();
		});

		it('should correctly extract child paths for objects', () => {
			expectTypeOf<Pxth<{ a: string; b: number }>['a']>().toEqualTypeOf<Pxth<string>>();
			expectTypeOf<Pxth<{ a: string; b: { c: number } }>['b']['c']>().toEqualTypeOf<Pxth<number>>();
			expectTypeOf<Pxth<Record<string, number>>['asdf']>().toEqualTypeOf<Pxth<number>>();
		});

		it('should respect null & undefined values for child paths in objects', () => {
			expectTypeOf<Pxth<{ a: string; b: number } | null>['a']>().toEqualTypeOf<Pxth<string | null>>();
			expectTypeOf<Pxth<{ a: string; b: { c: number } | undefined }>['b']['c']>().toEqualTypeOf<
				Pxth<number | undefined>
			>();
			expectTypeOf<Pxth<{ a: string | null | undefined }>['a']>().toEqualTypeOf<
				Pxth<string | null | undefined>
			>();
			expectTypeOf<Pxth<{ a: { b: Date | undefined } | null }>['a']['b']>().toEqualTypeOf<
				Pxth<Date | null | undefined>
			>();
		});
		...
		it('should correctly handle tuples', () => {
			expectTypeOf<Pxth<[string, number]>[0]>().toEqualTypeOf<Pxth<string>>();
			expectTypeOf<Pxth<[string, number]>[1]>().toEqualTypeOf<Pxth<number>>();
			expectTypeOf<Pxth<[Date]>[0]>().toEqualTypeOf<Pxth<Date>>();
		});

		it('should correctly handle null & undefined in arrays', () => {
			expectTypeOf<Pxth<string[] | null>[0]>().toEqualTypeOf<Pxth<string | null>>();
			expectTypeOf<Pxth<{ a: string | undefined }[]>[0]['a']>().toEqualTypeOf<Pxth<string | undefined>>();
			expectTypeOf<Pxth<(string | null)[] | undefined>[0]>().toEqualTypeOf<Pxth<string | null | undefined>>();
		});

		it('should correctly handle union types', () => {
			expectTypeOf<Pxth<{ a: string } | { a: number }>['a']>().toEqualTypeOf<Pxth<string | number>>();
		});
