...
export const useArrayControl = <V>({ name }: ArrayControlConfig<V>): ArrayControl<V> => {
	...
	const setItem = useReferenceCallback((item: V, index: number) =>
		updateArray(
			<T>(items: Array<T>) => {
	...
	const push = useReferenceCallback((...newItems: Array<V>) =>
		updateArray(
			<T>(oldItems: Array<T>) => {
	...
	const pop = useReferenceCallback(() =>
		updateArray(
			<T>(oldItems: Array<T>) => {
	...
	const removeAt = useReferenceCallback((index: number) =>
		updateArray(
			(items) => {
	...
	const swap = useReferenceCallback((first: number, second: number) =>
		updateArray(
			(items) => {
				const a = items[first];
	...
	const shift = useReferenceCallback(() =>
		updateArray(
			(items) => {
	...
	const unshift = useReferenceCallback((newItem: V) =>
		updateArray(
			<T>(items: Array<T>) => {
	...
	const insert = useReferenceCallback((index: number, value: V) =>
		updateArray(
			<T>(items: Array<T>) => {
